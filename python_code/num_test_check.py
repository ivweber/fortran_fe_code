...
# Use a discontinuous function on periodic domain [0,1], which has a zero 
# derivative. Use 6 cells and test various values of p for the Hermite and Lagrange 
# methods, with various maximum time-steps. Impose initial
# conditions weakly for better generality of method
...
def error_norm(U, t_curr, M, N, q, fem_type = "lagrange", quad_pts = 101, input_array = None):
	...
	# e^2 = (u,u) + (U,U) - 2(u,U) = (u0, u0) - 2(u,U) + U^T M U
...
5 for Hermite-DG or any combination thereof:""", end = " ")
	...
	for si in range(0, lsolv):
		...
		if s == "hermite_dg":
			A = sp.lil_matrix( (2 * N * q_2, 2 * N * q_2) )
			offset = 2 * q_2

		else:
			A = sp.lil_matrix( (N * q_2, N * q_2) )
			offset = q_2
